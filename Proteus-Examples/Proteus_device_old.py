...
#from src.Controller.ProteusUploader import ProteusUploader
...
class ProteusDriver:
    ...
    def __init__(self, ip_address: str, port: int = _PORT):
    ...
    def _connect(self):
        try:
            ...
            self.inst = self.admin.open_instrument(slot_id=sid)"""
            # if using LAN (slower)
            ...
            self.chan_list, self.segm_list = self.get_channels(self.model_name, self.sampling_rate)
    ...
    def apply_sampling_configuration(self, sampling_rate, interpol, dac_mode):
        """
    ...
    def send_wfm_to_proteus(self, samplingRate, channel, segment, myWfm, dacRes, initialize=False):
        try:
            ...
            self.send_command(':FUNC:MODE:SEGM {0}'.format(segment))
            if initialize:
    ...
    def _close(self):
        print(self.admin.close_all_instruments())
    ...
    def set_function_generator(self, channel, function='SIN', frequency=10e6,
        ...
        cycles = round(frequency * segLen / baseband_rate)  # Use baseband_rate
        ...
        quantized_wfm = self.my_quantization(dacWave, dac_mode, 0)
        max_dac = np.max(quantized_wfm)  # Max Dac
        ...
        try:
            ...
            self.send_wfm_to_proteus(samplingRate=target_sampling_rate, channel=channel, segment=1, myWfm=dacWave,
    ...
    def set_marker(self, marker):
        ...
        self.send_command(f':MARK SEL {int(marker)}')
    ...
    def set_marker_voltage(self, voltage):
        ...
        if voltage >0 or voltage<32:
            raise ValueError(f"Invalid voltage: {voltage}")
        self.send_command(f':MARK:VOL:LEV {voltage}')

    def get_marker_voltage(self):
        return self.send_command(f':MARK:VOL:LEV?', query=True)
    ...
    def set_enabling_task_signal(self, signal):
        ...
        if signal not in ["NONE", "TRIG1", "TRIG2", "TRIG3", "TRIG4", "TRIG5", "TRIG6", "INT", "CPU", "FBTR", "ANY"]:
            raise ValueError(f"Invalid signal: {signal}")
        if (signal == "TRIG3" or signal == "TRIG4") and (not "P1288D" in self.model or not "P12812D" in self.model or not "P2588D" in self.model or not "P25812D" in self.model or not "P1288B" in self.model or not "P12812B" in self.model or not "P2588B" in self.model or not "P25812B" in self.model or not "P9084D" in self.model or not "P9086D" in self.model or not "P9084B" in self.model or not "P9086B" in self.model):
            raise ValueError(f"Invalid signal: {signal} for model {self.model}")
        if (signal == "TRIG5" or signal == "TRIG6") and (not "P12812D" in self.model or not "P25812D" in self.model or not "P12812B" in self.model or not "P25812B" in self.model or not "P9086D" in self.model or not "P9086B" in self.model):
    ...
    def write_composer_array_to_task_table(self, offset):
        ...
        if offset < 1 or offset > 64000:
            raise ValueError(f"Invalid offset: {offset}")
        self.send_command(f':TASK:COMP:WRIT {offset}')
    ...
    def delete_all_segment(self):
        ...
        self.send_command(f":TRAC:DEL:ALL")
...
class ProteusDevice(Device):
    ...
    _DEFAULT_SETTINGS = Parameter([
        Parameter('ip_addess', _IP_ADDRESS, str, 'ip address'),
        Parameter('seq_file', 'scan.seq', str, 'Sequence file to upload to the Proteus'),
    ])
    ...
    def __init__(self, name=None, settings=None):
        ...
        self.logger = logging.getLogger(__name__)
        cfg = self.settings
        self.driver = ProteusDriver(_IP_ADDRESS)
    ...
    def __init__(
        ...
        self.uploader = None
        self.compiled_sequence = None

    """def connect(self):
        self.uploader = ProteusUploader(
            ip=self.ip,
    ...
    def program_sequence(self, compiled_sequence: CompiledSequence):
        if self.uploader is None:
            self.connect()
        self.compiled_sequence = compiled_sequence
        self.uploader.upload_compiled_sequence(compiled_sequence, first_task=1)"""

    def start(self):
        """Start task-mode sequence playback."""
        if self.uploader is None:
            raise RuntimeError("ProteusUploader not initialized")
        self.uploader.start()
...
if __name__ == "__main__":
    ...
    dev.set_function_generator(1, 'SIN')
    #dev.set_function_generator(4, 'SIN')
    dev.set_function_generator(3, 'RAMP', 2e6)
